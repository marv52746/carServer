const { ReservationDb } = require('../model/cars'); // Adjust the path as needed

const reservationController = {
    createReservation: async (req, res) => {
        const { car, customer, rentalStartDate, rentalEndDate, pickupLocation, dropoffLocation, rentalPrice, reservationNumber } = req.body;
    ...
    deleteReservation: async (req, res) => {
        try {
        const deletedReservation = await ReservationDb.findByIdAndDelete(req.params.id);
    ...
    getReservationbyFilter: async (req, res) => {
        ...
        if (startDate && endDate) {
            filter.rentalStartDate = { $gte: new Date(startDate), $lte: new Date(endDate) };
        }
